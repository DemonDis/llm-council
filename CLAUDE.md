...
**Стили (`*.css`)**
- Светлая и тёмная темы через CSS-переменные: значения в `:root` (светлая) переопределяются селектором `html[data-theme='dark']`; все компонентные стили используют только переменные
- Тема хранится в localStorage (`llm_council_theme`); по умолчанию — системная (`prefers-color-scheme`); применяется атрибутом на `<html>` ещё до рендера (`main.jsx`), чтобы не было вспышки светлой темы; переключение — анимированная SVG-иконка солнце/луна в шапке сайдбара рядом с заголовком «LLM Council» (`Sidebar.jsx`, состояние в `App.jsx`)
