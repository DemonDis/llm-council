# LLM Council

Идея этого репозитория в том, что вместо того, чтобы задавать вопрос вашему любимому LLM-провайдеру (например, OpenAI GPT 5.1, Google Gemini 3.0 Pro, Anthropic Claude Sonnet 4.5, xAI Grok 4 и т.д.), вы можете объединить их в свой "Совет LLM" (LLM Council). Это простое локальное веб-приложение, которое по сути выглядит как ChatGPT, но использует OpenRouter для отправки вашего запроса сразу нескольким LLM. Затем модели просят оценить и проранжировать работы друг друга, и в конце председатель — LLM — формирует итоговый ответ.

## Два режима работы

Приложение поддерживает два режима, которые переключаются в интерфейсе над полем ввода:

**Режим «Битва моделей» (Ensemble, по умолчанию):** один вопрос задаётся разным моделям. Подробнее о том, что происходит при отправке запроса:

1. **Этап 1: Первые мнения**. Запрос пользователя отправляется всем LLM по отдельности, и ответы собираются. Каждый ответ отображается во "вкладках", чтобы пользователь мог просмотреть их по одному.
2. **Этап 2: Рецензирование**. Каждой LLM передаются ответы других LLM. Под капотом личности LLM анонимизируются, чтобы модель не могла играть в любимчиков, оценивая свои собственные результаты. Модель просят проранжировать ответы по точности и глубине анализа.
3. **Этап 3: Итоговый ответ**. Назначенный Председатель Совета LLM собирает все ответы моделей и компилирует их в единый финальный ответ, который и показывается пользователю.

**Режим «Ролевой мозговой штурм» (Roleplay):** вместо набора моделей один вопрос задаётся ролям в одной модели. Роли — ваши «цифровые личности» (например, коллеги по работе или персонажи вроде Рика Санчеса). Каждая роль получает свой системный промпт:

1. **Этап 1: Мнения ролей**. Вопрос отправляется каждой роли — каждая отвечает со своей точки зрения.
2. **Этап 2: Рецензирование**. Каждая роль (в своём характере) анонимно оценивает и ранжирует ответы остальных ролей.
3. **Этап 3: Итоговый ответ**. Председатель синтезирует единое взвешенное решение с учётом всех мнений.

Модель для ролей настраивается переменной `ROLEPLAY_MODEL` в `.env`.

### Настройка ролей (`roles.json`)

Роли редактируются в файле `roles.json` в корне проекта (без правки кода):

- Формат: `{"Имя роли": "системный промпт, описывающий, как эта личность думает и говорит", ...}`.
- Опишите личность как можно конкретнее: кто это, чем отличается, стиль речи. Для персонажей (Рик Санчес) опишите характер, манеру и ограничения.
- Ключи, начинающиеся с `_`, игнорируются — удобно для заметок внутри файла.
- После изменения файла перезапустите бэкенд. Если файла нет или он пуст — используются встроенные роли по умолчанию.

## Установка

### 1. Установка зависимостей

Проект использует [uv](https://docs.astral.sh/uv/) для управления.

**Бэкенд:**
```bash
uv sync
```

**Фронтенд:**
```bash
cd frontend
npm install
cd ..
```

### 2. Настройка API-ключа

Создайте файл `.env` в корне проекта:

```bash
OPENROUTER_API_KEY=sk-or-v1-...
```

Получите свой API-ключ на [openrouter.ai](https://openrouter.ai/). Не забудьте пополнить баланс на нужную сумму или включить автоматическое пополнение.

### 3. Настройка моделей (необязательно)

Отредактируйте `.env`, чтобы настроить совет:

```bash
# Члены совета — список идентификаторов моделей OpenRouter через запятую
COUNCIL_MODELS=deepseek-v3.1,gemini-3-pro,gemma-3-27b,glm-4.6,gpt-5.1,gpt-oss-120b,grok-4,qwen3-30b-a3b

# Модель председателя — синтезирует итоговый ответ
CHAIRMAN_MODEL=gemini-3-pro

# Модель для ролевого режима — отвечает от лица всех ролей совета
ROLEPLAY_MODEL=gemini-3-pro
```

Пример с единственным членом совета и другим председателем:

```bash
COUNCIL_MODELS=openai/gpt-5.1
CHAIRMAN_MODEL=google/gemini-3-pro-preview
```

## Запуск приложения

**Вариант 1: Использовать стартовый скрипт**
```bash
./start.sh
```

**Вариант 2: Запустить вручную**

Терминал 1 (Бэкенд):
```bash
uv run python -m backend.main
```

Терминал 2 (Фронтенд):
```bash
cd frontend
npm run dev
```

Затем откройте http://localhost:5173 в браузере.

## Технологический стек

- **Бэкенд:** FastAPI (Python 3.10+), async httpx, OpenRouter API
- **Фронтенд:** React + Vite, react-markdown для рендеринга
- **Хранилище:** JSON-файлы в `data/conversations/`
- **Управление зависимостями:** uv для Python, npm для JavaScript
